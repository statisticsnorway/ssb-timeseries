...
def combinations(ds, attributes = [], taxonomies = [], all_attributes = [], sep='.'):
    """
    Lists possible combinations from dataset based on attribute tags and taxonomies.

    Args:
        ds: dataset
        attributes: attributes for aggregation
        taxonomies: taxonomies for aggregation
    """
    st = ds.series_tags
    if all_attributes == []:
        all_attributes = ds.auto_tag_config["attributes"]
    combinations_ds = pd.DataFrame([st[x] for x in st])[all_attributes]

    map_dict = dict(zip(attributes, taxonomies))

    for x in map_dict:
        combinations_ds[x] = combinations_ds[x].map(map_dict[x].code_dict)
        combinations_ds = combinations_ds.explode(x)

    exploded = combinations_ds.dropna().drop_duplicates()
    
    # Konstruerer serienavn til aggregerte serier
    exploded["new_series_name"] = exploded[all_attributes].astype(str).apply(lambda x: sep.join(x), axis=1)
    
    # Mapper grunnkodene tilbake for å kjøre select
    for x in map_dict:
        exploded[x] = exploded[x].map(map_dict[x].agg_dict)
    return exploded.to_dict('records')


# TODO: "ds" erstattes med "self" hvis denne skal inn i Dataset-klassen - søk og erstatt på "ds_replace_self"
# TODO: option for å droppe suffix/prefix/endre formatering
def aggregate_ds(
        ds_replace_self,
        attributes: list[str],
        taxonomies: list[int | Taxonomy | dict[str, str]],
        functions: list = ["sum"], #: list[str | F] | set[str | F],
        sep: str = "_",
        all_attributes = []
    ):
    """
    Alternative aggregation function based on a list of existing combinations of attributes in the dataset series.
    """
    new_series_list = combinations(ds_replace_self, attributes, taxonomies, all_attributes, sep)
    
    # Beregner aggregerte serier
    # select bruker tid her
    new_ds = nw.from_native(ds_replace_self.data).drop(ds_replace_self.numeric_columns).to_pandas()
    for x in new_series_list:
        criteria = {k: v for k, v in x.items() if k != 'new_series_name'}
        leaf_node_subset = ds_replace_self.select(tags=criteria, output="df")
        for func in functions:
            new_ds[x["new_series_name"] + f"{sep}{func}"] = column_aggregate(leaf_node_subset, func)

    # TODO: Return timeseries dataset
    return new_ds